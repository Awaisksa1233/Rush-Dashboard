...
import { Car, Clock, ShieldAlert, Sparkles, Gauge, Zap, TrendingUp, AlertTriangle, Database } from 'lucide-react';
    ...
    <div className="space-y-6 font-sans select-none">
      ...
      */}
      <div className="glass-card rounded-2xl p-5 border border-slate-200/90 shadow-2xs">
        ...
        {/* 4 Usage Tier Cards */}
        {(!usage?.usageTiers || usage.usageTiers.length === 0) ? (
          <div className="text-center py-12 text-slate-400">
            <Database className="w-8 h-8 mx-auto mb-3 text-slate-300" />
            <p className="text-sm font-medium">No usage tier data available</p>
      ...
      <div className="grid grid-cols-1 lg:grid-cols-12 gap-4">
        ...
        <div className="lg:col-span-7 glass-card rounded-2xl p-5 border border-slate-200/90 shadow-2xs flex flex-col justify-between">
          <div>
            ...
            {(!usage?.peakHours || usage.peakHours.length === 0) ? (
              <div className="text-center py-12 text-slate-400">
                <Database className="w-8 h-8 mx-auto mb-3 text-slate-300" />
        ...
        <div className="lg:col-span-5 glass-card rounded-2xl p-5 border border-slate-200/90 shadow-2xs flex flex-col justify-between">
          <div>
            ...
            {(!usage?.vehicleTypes || usage.vehicleTypes.length === 0) ? (
              <div className="text-center py-12 text-slate-400">
                <Database className="w-8 h-8 mx-auto mb-3 text-slate-300" />
                <p className="text-sm font-medium">No vehicle type data available</p>
                <p className="text-xs mt-1">Data will appear here when available in the database</p>
              </div>
            ) : (
